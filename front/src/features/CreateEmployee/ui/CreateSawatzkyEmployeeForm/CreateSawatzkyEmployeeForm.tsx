...
import { editSawatzkyEmployee } from 'features/CreateEmployee/model/services/editSawayzkyEmployee';
import { EmployeeRoleOption } from '../../model/type/createEmployee';
import {
  getCreateEmployeeChangePasswordData,
  getCreateEmployeeId,
  getCreateEmployeeIsEdit,
  getCreateEmployeeNewPassword,
  getCreateEmployeeOldPassword,
...
export const CreateSawatzkyEmployeeForm: React.FC<CreateSawatzkyEmployeeFormProps> = (props) => {
  ...
  const changePasswordData = useSelector(getCreateEmployeeChangePasswordData);
  const newPassword = useSelector(getCreateEmployeeNewPassword);
  const oldPassword = useSelector(getCreateEmployeeOldPassword);
  ...
  const onSubmitForm = useCallback(() => {
    if (isEdit && employeeId) {
      if (changePasswordData && Object.values(changePasswordData).every((value) => value !== '')) {
        //
      } else {
        //
      }
  ...
  }, [changePasswordData, dispatch, formData, user, isEdit, employeeId]);
  ...
  const { Form } = useForm({
    fields: [
      ...
      isEdit ? {
        ...
        value: newPassword ?? '',
        onChange: (value: string) => dispatch(createEmployeeActions.setNewPassword(value)),
        rules: {
          required: false,
        },
      } : null as any,
      isEdit ? {
        ...
        value: oldPassword ?? '',
        onChange: (value: string) => dispatch(createEmployeeActions.setOldPassword(value)),
        rules: {
          required: false,
          equal: {
            id: 'new_password',
