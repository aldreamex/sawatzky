...
const DirectoryObjectsGroupPage: React.FC<DirectoryObjectsGroupPageProps> = (props) => {
  ...
  const onTableEditHandler = useCallback((item: TableItemType) => {
    const workObjectGroup = workObjectGroups.find((workObjectGroup) => workObjectGroup.id === item.id);
    if (workObjectGroup) {
      dispatch(addWorkObjectGroupFormActions.openEditModal(workObjectGroup));
    }
  }, [dispatch, workObjectGroups]);
