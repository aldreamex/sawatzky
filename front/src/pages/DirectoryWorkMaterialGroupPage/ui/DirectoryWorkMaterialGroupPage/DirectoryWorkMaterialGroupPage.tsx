...
const DirectoryWorkMaterialGroupPage: React.FC<DirectoryObjectsGroupPageProps> = (props) => {
  ...
  const onTableEditHandler = useCallback((item: TableItemType) => {
    const workMaterialGroup = workObjectsList.find((workMaterialGroup) => workMaterialGroup.id === item.id);
    if (workMaterialGroup) {
      dispatch(addWorkMaterialGroupFormActions.openEditModal(workMaterialGroup));
    }
